...
export class TeachersService {
  constructor(
    @InjectModel(Teachers) private model: typeof Teachers,
  ...
  async signinTeacher(signinTeacherDto: SigninTeacherDto): Promise<object> {
    try {
      ...
      if (!teacher)
        throw new BadRequestException('Email adress or password incorrect');
  ...
  async confirmSignin(
    confirmSigninTeacherDto: ConfirmSigninTeacherDto,
    res: Response,
  ) {
    try {
      ...
      if (!istureOtp || istureOtp !== otp)
        throw new BadRequestException('OTP experid');
  ...
  async remove(id: number): Promise<object | undefined> {
    try {
      if (!(await this.findTeacherById(id)))
        throw new NotFoundException('Teacher not found');

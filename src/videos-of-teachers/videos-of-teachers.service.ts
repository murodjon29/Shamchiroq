...
export class VideosOfTeachersService {
  constructor(
    @InjectModel(Videos_of_teachers) private model: typeof Videos_of_teachers,
    private readonly fileService: FileService,
  ) {}

  async create(
    createVideosOfTeacherDto: CreateVideosOfTeacherDto,
    ...
    try {
      ...
      const videos_of_teachers = await this.model.create({
  ...
  async findAll(): Promise<object> {
    try {
      const videos_of_teachers = await this.model.findAll({
        include: { all: true },
      });
      return successRes(videos_of_teachers);
    } catch (error) {
  ...
  async update(
    id: number,
    updateVideosOfTeacherDto: UpdateVideosOfTeacherDto,
    file?: Express.Multer.File,
  ): Promise<object> {
    try {
      ...
      const updateVideo = await this.model.update(
        { ...updateVideosOfTeacherDto, video },
  ...
  async findTeacherVideoById(id: number): Promise<object> {
    try {
      const teacher_videos = await this.model.findByPk(id);
      if (!teacher_videos)
        throw new NotFoundException('Teacher videos not found');
